(function(root) {
    ...
    Inventory.prototype = {
         ...
         */
        addByType: function(type, quantity, settings){
            quantity = quantity || 1;

            for (var i = 0; i < quantity; i++) {
                var item  = RL.Item.make(this.game, type, settings);
                this.add(item);
            }
        },
        ...
        /**
         * Retrieves the first item of given type found and removes it.
         * @method getFirstByType
         * @param {String} type - A string to lookup an type `RL.Item.Types[type]`.
         * @return {Bool|Item} False if no results found.
         */
        removeFirstByType: function(type){
            for (var i = 0; i < this.items.length; i++) {
                var item = this.items[i];
                if(item.type === type){
        ...
        /**
         * Retrieves the last item of given type found and removes it.
         * @method getLastByType
         * @param {String} type - A string to lookup an type `RL.Item.Types[type]`.
         * @return {Bool|Item} False if no results found.
         */
        removeLastByType: function(type){
            for(var i = this.items.length - 1; i >= 0; i--){
                var item = this.items[i];
                if(item.type === type){
                    return this.remove(item);
